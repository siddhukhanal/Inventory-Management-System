"use client";
import { ExpenseByCategorySummary, useGetExpensesByCategoryQuery } from "@/src/state/api";
...
const Expenses = () => {
    ...
    const{data:expensesData, isLoading,isError,
    ...
    const aggregatedData : AggregatedDataItem[]= useMemo(()=>{
        ...
        if(!acc[data.category]){
            ...
            acc[data.category].color=`#${Math.floor(Math.random()*166777215).toString(16)}`
                    ...
                    >
                        {aggregatedData.map(
                            (entry:AggregatedDataItem, index:number)=> (
                                <Cell key={`cell-${index}`} fill={index==activateIndex ? "rgb(29,78,216)":entry.color}/>
                            )
                        )}
